...
# ORIGINAL comprehensive list of words to replace, including specific variations.
# This list will be used to build the replacement logic.
WORDS_TO_REPLACE = [
    (" The hell ", " The heck "),
    (" The hell! ", " The heck! "),
    (" The hell? ", " The heck? "),
    ...
    (" Fuck! ", " Fetch! "),
    (" Fuck, ", " Fetch, "),
    (" Fuck ", " Fetch "), # Added this back as a base case if no punc/space
    ...
    (" Crap! ", " Crud! "),
    (" Crap, ", " Crud, "),
    (" Crap ", " Crud "), # Added this back as a base case if no punc/space
    (" Craphole ", " Trash heap "),
    (" Craphole! ", " Trash heap! "),
    (" Craphole, ", " Trash heap, "),
    (" Crapped ", " Pooped "),
...
class PiperTTSGUI:
    ...
    def convert_files(self):
        ...
        try:
            ...
            for filename in os.listdir(input_dir):
                ...
                # Do the replace *before* inserting into the f-string expression to avoid SyntaxError
                ...
                command = (
                    f'bash -c "MSYS2_ARG_CONV_EXCL=\\"*\\" cat \\"{input_path_bash}\\" | ' # Use the pre-processed path variable
                    f'\\"{piper_executable_bash}\\" ' # Use the pre-processed path variable
                    f'--model \\"{model_path_bash}\\" --length_scale {length_scale:.2f} --sentence_silence {sentence_silence:.2f} '
                    f'--output_file \\"{output_path_bash}\\""' # Use the pre-processed path variable
                )
        ...
        except Exception as e: # This should be correctly aligned with the initial 'try'
    ...
    def split_text_file(self):
        ...
        try:
            ...
            if not words_per_file_str or int(words_per_file_str) < 1: # Validate range
                 messagebox.showerror("Error", "Words per file must be at least 1.")
                 return
